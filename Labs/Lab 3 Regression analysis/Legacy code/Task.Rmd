...
df_lab3 <- df_lab3 %>%
  mutate(Gender = dplyr::recode(Gender, "M" = 0, "F" = 1))
...
model_1 <- lm(Total.Charges ~ Length.of.Stay + APR.Severity.of.Illness.Code, data = df_lab3)
model_hc1_1 <- coeftest(model_1, vcov. = hccm(model_1, type = "hc1"))

# stargazer(model_hc1, model_hc1_1, type = "text", digits = 3)

model_2 <- lm(Total.Charges ~ Length.of.Stay + APR.Severity.of.Illness.Code
              + Gender, data = df_lab3)
model_hc1_2 <- coeftest(model_2, vcov. = hccm(model_2, type = "hc1"))

# stargazer(model_hc1_1, model_hc1_2, type = "text", digits = 3)
cov(df_lab3$Length.of.Stay, df_lab3$APR.Severity.of.Illness.Code)
var(df_lab3$Length.of.Stay)
cov(df_lab3$Length.of.Stay, df_lab3$APR.Severity.of.Illness.Code) / var(df_lab3$Length.of.Stay)
...
# Визначення спостережених значень та прогнозованих значень
# observed <- df$Total.Charges
# predicted <- predict(model)
# 
# # Графік спостережених значень та прогнозованих значень
# plot(observed, predicted, main = "Графік спостережених та прогнозованих значень", xlab = "Спостережені", ylab = "Прогнозовані")
